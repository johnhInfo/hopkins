 ...
 * Adapted for Foundations of Algorithms Johns Hopkins University by Garry Binder
 */
...
public class TicTacToeFrame extends JFrame {
	...
	private byte[][] moves = { { 1, 1 }, { 0, 0 }, { 0, 2 }, { 2, 0 }, { 2, 2 }, { 0, 1 }, { 1, 0 }, { 1, 2 }, { 2, 1 } };
	...
	private void movePlayer(int mouseX, int mouseY) {
		...
		if (currentState == GameState.PLAYING) {
			if (rowSelected >= 0 && rowSelected < ROWS && colSelected >= 0 && colSelected < COLS
				...
				if (currentPlayer == Seed.CROSS && !hasWon(Seed.NOUGHT, rowSelected, colSelected, board)) {
					byte[] move = bestMove();
					if (move[0] != -1)
						movePlayer(move[1] * CELL_SIZE, move[0] * CELL_SIZE);
				}
	...
	public void updateGame(Seed theSeed, int rowSelected, int colSelected) {
		...
		if (hasWon(theSeed, rowSelected, colSelected, board)) { // check for win
	 ...
	 * (rowSelected, colSelected)
	 */
	public boolean hasWon(Seed theSeed, int rowSelected, int colSelected, Seed[][] board) {
	...
	private byte[] bestMove() {
		...
		loop: for (int i = 0; i < 9; i++) {
			byte[] moveCross = moves[i];

			if (board[moveCross[0]][moveCross[1]] == Seed.EMPTY) {

				Seed[][] fakeBoard = new Seed[3][3];
				for (int row = 0; row < 3; row++) {
					for (int col = 0; col < 3; col++)
						fakeBoard[row][col] = board[row][col];
				}

				fakeBoard[moveCross[0]][moveCross[1]] = Seed.CROSS;
				if (hasWon(Seed.CROSS, moveCross[0], moveCross[1], fakeBoard)) {
					return moveCross;
				}

				for (int j = 0; j < 9; j++) {
					byte[] moveNought = moves[j];

					if (fakeBoard[moveNought[0]][moveNought[1]] == Seed.EMPTY) {

						fakeBoard[moveNought[0]][moveNought[1]] = Seed.NOUGHT;
						if (hasWon(Seed.NOUGHT, moveNought[0], moveNought[1], fakeBoard)) {
							continue loop;
						}
						fakeBoard[moveNought[0]][moveNought[1]] = Seed.EMPTY;
					}
				}
